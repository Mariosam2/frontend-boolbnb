    ...
    methods: {
        async search() {
            try {
                ...
                const response = await axios.get('https://boolbnb-host.com/api/search?address=' + store.address + '&services=' + store.services_back + '&category=' + store.categories_back + '&radius=' + store.radius * 1000 + '&beds=' + store.beds);
